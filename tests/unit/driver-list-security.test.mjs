...
test("production status control follows the company-admin permission", () => {
    assert.match(source, /IS_DEMO_MODE \|\| window\.currentUser\?\.role === "company-admin"/);
...
test("driver status uses the authenticated server API outside demo mode", () => {
    ...
    assert.match(source, /if \(IS_DEMO_MODE\) saveState\(\)/);
});

test("manual driver create and edit stay demo-only", () => {
    assert.match(source, /function addDriver\(event\) \{[\s\S]*?if \(!IS_DEMO_MODE\) return;/);
    assert.match(source, /function editDriver\(id\) \{[\s\S]*?if \(!IS_DEMO_MODE\) return;/);
    assert.match(source, /function importDriversBulk\(\) \{[\s\S]*?if \(!IS_DEMO_MODE\) return;/);
});
