/**
 * FAZA 3 D24.2 packer — body-hash manifest + review ⊆ full byte identity.
 */
...
const logs = join(reports, "phase-3-d242-logs");
mkdirSync(logs, { recursive: true });
const baseSha = "a6fbcb508c67287c33479f38c3678cd44684ee60";
const MANIFEST_NAME = "reports/phase-3-d242-source-manifest.txt";

function sha256(buf) { return createHash("sha256").update(buf).digest("hex"); }
function toPosix(rel) { return rel.split(sep).join("/"); }

const visualLatest = existsSync(join(reports, "phase-3-d242-visual-latest.txt"))
  ? readFileSync(join(reports, "phase-3-d242-visual-latest.txt"), "utf8").trim()
  : "";
const visualRel = visualLatest
  ? toPosix(relative(root, visualLatest))
  : "reports/phase-3-d242-visual";
...
function shouldSkipFile(name, relPosix) {
  ...
  if (name.startsWith("_phase3-") || name.startsWith("_tmp-") || name.startsWith("_phase3-d242-")) return true;
  if (relPosix === "reports/phase-3-d242-logs/pack.txt") return true;
  if (relPosix === "reports/phase-3-d242-logs/manifest-verifier.txt") return true;
...
writeFileSync(join(reports, "phase-3-d242-git-status-short.txt"), status.stdout || "");
writeFileSync(join(reports, "phase-3-d242-git-diff-stat.txt"), diffStat.stdout || "");
writeFileSync(join(reports, "phase-3-d242-base-to-working.patch"), patch.stdout || "");
...
const bodyLines = [
  "PHASE 3 D24.2 - SOURCE MANIFEST BODY",
  "date: 2026-08-10",
  "base SHA:",
  baseSha,
...
`;
  const psFile = join(reports, "_phase3-d242-zip.ps1");
...
const reviewFiles = [
  ...
  "tests/e2e/dispo-soft-remove.spec.js",
  "scripts/phase3-d242-fail-first-race.mjs",
  "scripts/phase3-d242-visual-trail.mjs",
  "scripts/phase3-d242-pack-artifacts.mjs",
  "reports/phase-3-d242-report-2026-08-10.md",
  "reports/phase-3-d242-change-ledger.md",
  MANIFEST_NAME,
  "reports/phase-3-d242-git-status-short.txt",
  "reports/phase-3-d242-git-diff-stat.txt",
  "reports/phase-3-d242-base-to-working.patch"
];

const reviewList = [...new Set([
  ...reviewFiles.filter((r) => existsSync(join(root, r))),
  ...collectDir("reports/phase-3-d242-logs"),
...
const reviewZip = join(reports, "phase-3-d242-review-source-2026-08-10.zip");
const fullZip = join(reports, "phase-3-d242-full-deliverable-2026-08-10.zip");
for (const z of [reviewZip, fullZip]) {
  if (existsSync(z)) unlinkSync(z);
}
zipWithDotNet(reviewList, reviewZip, join(reports, "_phase3-d242-review-list.txt"));
zipWithDotNet(fullList, fullZip, join(reports, "_phase3-d242-full-list.txt"));
...
`;
  const f = join(reports, "_phase3-d242-listzip.ps1");
  writeFileSync(f, ps);
