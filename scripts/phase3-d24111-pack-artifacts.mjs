/**
 * FAZA 3 D24.1.1.1 packer — body-hash manifest + review ⊆ full byte identity.
 */
...
const logs = join(reports, "phase-3-d24111-logs");
mkdirSync(logs, { recursive: true });
const baseSha = "a6fbcb508c67287c33479f38c3678cd44684ee60";
const MANIFEST_NAME = "reports/phase-3-d24111-source-manifest.txt";

function sha256(buf) { return createHash("sha256").update(buf).digest("hex"); }
function toPosix(rel) { return rel.split(sep).join("/"); }

const visualLatest = existsSync(join(reports, "phase-3-d24111-visual-latest.txt"))
  ? readFileSync(join(reports, "phase-3-d24111-visual-latest.txt"), "utf8").trim()
  : "";
const visualRel = visualLatest
  ? toPosix(relative(root, visualLatest))
  : "reports/phase-3-d24111-visual";
...
function shouldSkipFile(name, relPosix) {
  ...
  if (name.startsWith("_phase3-") || name.startsWith("_tmp-") || name.startsWith("_phase3-d24111-")) return true;
  if (relPosix === "reports/phase-3-d24111-logs/pack.txt") return true;
  if (relPosix === "reports/phase-3-d24111-logs/manifest-verifier.txt") return true;
...
writeFileSync(join(reports, "phase-3-d24111-git-status-short.txt"), status.stdout || "");
writeFileSync(join(reports, "phase-3-d24111-git-diff-stat.txt"), diffStat.stdout || "");
writeFileSync(join(reports, "phase-3-d24111-base-to-working.patch"), patch.stdout || "");
...
const bodyLines = [
  "PHASE 3 D24.1.1.1 - SOURCE MANIFEST BODY",
  "date: 2026-08-09",
  "base SHA:",
  baseSha,
...
`;
  const psFile = join(reports, "_phase3-d24111-zip.ps1");
...
const reviewFiles = [
  ...
  "server/driver-credential-migration.js",
  ...
  "tests/unit/phase3-d2411-ca-drivers-http.test.js",
  "tests/unit/driver-credential-migration.test.js",
  "tests/e2e/qa-factory.js",
  "tests/e2e/ui-smoke.spec.js",
  "scripts/phase3-d24111-pack-artifacts.mjs",
  "scripts/phase3-d24111-visual-trail.mjs",
  "scripts/_d24111-scope-inner.js",
  "reports/phase-3-d24111-report-2026-08-09.md",
  "reports/phase-3-d24111-change-ledger.md",
  MANIFEST_NAME,
  "reports/phase-3-d24111-git-status-short.txt",
  "reports/phase-3-d24111-git-diff-stat.txt",
  "reports/phase-3-d24111-base-to-working.patch"
];

const reviewList = [...new Set([
  ...reviewFiles.filter((r) => existsSync(join(root, r))),
  ...collectDir("reports/phase-3-d24111-logs"),
...
// Review must be a byte-identical subset of full-listed content.
...
const reviewZip = join(reports, "phase-3-d24111-review-source-2026-08-09.zip");
const fullZip = join(reports, "phase-3-d24111-full-deliverable-2026-08-09.zip");
for (const z of [reviewZip, fullZip]) {
  if (existsSync(z)) unlinkSync(z);
}
zipWithDotNet(reviewList, reviewZip, join(reports, "_phase3-d24111-review-list.txt"));
zipWithDotNet(fullList, fullZip, join(reports, "_phase3-d24111-full-list.txt"));
...
`;
  const f = join(reports, "_phase3-d24111-listzip.ps1");
  writeFileSync(f, ps);
...
for (const rel of reviewList) {
  if (!fullEntries.includes(rel) && rel !== MANIFEST_NAME) {
    // review may include manifest
  }
