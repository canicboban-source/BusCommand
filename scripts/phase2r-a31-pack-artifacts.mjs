/**
 * FAZA 2R-A.3.1 packer — body-hash manifest + per-file SHA/size re-verify.
 * Circular/stale-log policy: pack.txt and *.tmp are excluded from body + ZIPs;
 * verifier log is written only after ZIP creation (authoritative for this run).
 */
...
const logs = join(reports, "phase-2r-a31-logs");
mkdirSync(logs, { recursive: true });
const baseSha = "a6fbcb508c67287c33479f38c3678cd44684ee60";
const MANIFEST_NAME = "reports/phase-2r-a31-source-manifest.txt";
...
function shouldSkipFile(name, relPosix) {
  ...
  if (name.startsWith("_phase2r-a31-") || name.startsWith("_tmp-")) return true;
  if (relPosix === "reports/phase-2r-a31-logs/pack.txt") return true;
  if (relPosix === "reports/phase-2r-a31-logs/manifest-verifier.txt") return true;
...
writeFileSync(join(reports, "phase-2r-a31-git-status-short.txt"), status.stdout || "");
writeFileSync(join(reports, "phase-2r-a31-git-diff-stat.txt"), diffStat.stdout || "");
writeFileSync(join(reports, "phase-2r-a31-base-to-working.patch"), patch.stdout || "");
...
const bodyLines = [
  "PHASE 2R-A.3.1 - SOURCE MANIFEST BODY",
  "date: 2026-08-09",
  "base SHA:",
  baseSha,
...
`;
  const psFile = join(reports, "_phase2r-a31-zip.ps1");
...
const reviewFiles = [
  ...
  "translations.js",
  "AGENTS.md",
  "tests/unit/phase2r-a31-fail-first.test.js",
  "tests/rules/phase2r-a31-cross-writer-atomicity.test.js",
  "tests/rules/phase2r-a3-commit-concurrency.test.js",
  "scripts/phase2r-a31-visual-trail.mjs",
  "scripts/phase2r-a31-pack-artifacts.mjs",
  "reports/phase-2r-a31-report-2026-08-09.md",
  "reports/phase-2r-a31-change-ledger.md",
  "reports/phase-2r-a31-writer-inventory.md",
  MANIFEST_NAME,
  "reports/phase-2r-a31-git-status-short.txt",
  "reports/phase-2r-a31-git-diff-stat.txt",
  "reports/phase-2r-a31-base-to-working.patch",
  "reports/git-status-short.txt",
  "reports/git-diff-stat.txt",
  "reports/base-to-working.patch",
  "reports/phase-2r-a31-visual/01-in-progress.png",
  "reports/phase-2r-a31-visual/02-in-progress-retry.png",
  "reports/phase-2r-a31-visual/03-recovery-required.png",
  "reports/phase-2r-a31-visual/04-xss-as-text.png",
  "reports/phase-2r-a31-visual/05-idempotent-success.png",
  "reports/phase-2r-a31-visual/TRAIL.json",
  "reports/phase-2r-a31-visual/README.md"
];

const reviewList = [...new Set([
  ...reviewFiles.filter((r) => existsSync(join(root, r))),
  ...collectDir("reports/phase-2r-a31-logs"),
  ...collectDir(".cursor/rules")
])].sort();
const fullList = [...new Set([...allSource, MANIFEST_NAME])].sort();

const reviewZip = join(reports, "phase-2r-a31-review-source-2026-08-09.zip");
const fullZip = join(reports, "phase-2r-a31-full-deliverable-2026-08-09.zip");
for (const z of [reviewZip, fullZip]) {
  if (existsSync(z)) unlinkSync(z);
}
zipWithDotNet(reviewList, reviewZip, join(reports, "_phase2r-a31-review-list.txt"));
zipWithDotNet(fullList, fullZip, join(reports, "_phase2r-a31-full-list.txt"));

// Independent verifier: recompute each listed file hash+size; open both ZIPs.
const seen = new Set();
const dupes = [];
...
`;
  const f = join(reports, "_phase2r-a31-listzip.ps1");
  writeFileSync(f, ps);
