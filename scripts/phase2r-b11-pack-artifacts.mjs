/**
 * FAZA 2R-B.1.1 packer — body-hash manifest + per-file SHA/size re-verify.
 */
...
const logs = join(reports, "phase-2r-b11-logs");
mkdirSync(logs, { recursive: true });
const baseSha = "a6fbcb508c67287c33479f38c3678cd44684ee60";
const MANIFEST_NAME = "reports/phase-2r-b11-source-manifest.txt";
...
function shouldSkipFile(name, relPosix) {
  ...
  if (name.startsWith("_phase2r-b11-") || name.startsWith("_tmp-")) return true;
  if (relPosix === "reports/phase-2r-b11-logs/pack.txt") return true;
  if (relPosix === "reports/phase-2r-b11-logs/manifest-verifier.txt") return true;
...
writeFileSync(join(reports, "phase-2r-b11-git-status-short.txt"), status.stdout || "");
writeFileSync(join(reports, "phase-2r-b11-git-diff-stat.txt"), diffStat.stdout || "");
writeFileSync(join(reports, "phase-2r-b11-base-to-working.patch"), patch.stdout || "");
...
const bodyLines = [
  "PHASE 2R-B.1.1 - SOURCE MANIFEST BODY",
  "date: 2026-08-09",
  "base SHA:",
  baseSha,
...
`;
  const psFile = join(reports, "_phase2r-b11-zip.ps1");
...
const reviewFiles = [
  "js/dispatcher/plan-import-loader.js",
  "js/register-onclick-staff.js",
  "js/dispatcher/plan-import.js",
  "translations.js",
  "AGENTS.md",
  "tests/unit/phase2r-b11-file-recovery.test.mjs",
  "tests/unit/phase2r-b1-lazy-chunk-recovery.test.mjs",
  "tests/unit/phase2r-a311-confirm-lock-consistency.test.js",
  "tests/e2e/phase2r-b11-file-recovery.spec.js",
  "tests/fixtures/qa-monthly-plan-import-loose.txt",
  "scripts/phase2r-b11-visual-trail.mjs",
  "scripts/phase2r-b11-pack-artifacts.mjs",
  "reports/phase-2r-b11-report-2026-08-09.md",
  "reports/phase-2r-b11-change-ledger.md",
  MANIFEST_NAME,
  "reports/phase-2r-b11-git-status-short.txt",
  "reports/phase-2r-b11-git-diff-stat.txt",
  "reports/phase-2r-b11-base-to-working.patch",
...
const reviewList = [...new Set([
  ...
  ...collectDir("reports/phase-2r-b11-logs"),
  ...collectDir("reports/phase-2r-b11-visual"),
  ...collectDir(".cursor/rules")
])].sort();
const fullList = [...new Set([...allSource, MANIFEST_NAME])].sort();

const reviewZip = join(reports, "phase-2r-b11-review-source-2026-08-09.zip");
const fullZip = join(reports, "phase-2r-b11-full-deliverable-2026-08-09.zip");
for (const z of [reviewZip, fullZip]) {
  if (existsSync(z)) unlinkSync(z);
}
zipWithDotNet(reviewList, reviewZip, join(reports, "_phase2r-b11-review-list.txt"));
zipWithDotNet(fullList, fullZip, join(reports, "_phase2r-b11-full-list.txt"));
...
`;
  const f = join(reports, "_phase2r-b11-listzip.ps1");
  writeFileSync(f, ps);
