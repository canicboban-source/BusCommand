/**
 * FAZA 2R-B packer — body-hash manifest + per-file SHA/size re-verify.
 */
...
const logs = join(reports, "phase-2r-b-logs");
mkdirSync(logs, { recursive: true });
const baseSha = "a6fbcb508c67287c33479f38c3678cd44684ee60";
const MANIFEST_NAME = "reports/phase-2r-b-source-manifest.txt";
...
function shouldSkipFile(name, relPosix) {
  ...
  if (name.startsWith("_phase2r-b-") || name.startsWith("_tmp-")) return true;
  if (relPosix === "reports/phase-2r-b-logs/pack.txt") return true;
  if (relPosix === "reports/phase-2r-b-logs/manifest-verifier.txt") return true;
...
writeFileSync(join(reports, "phase-2r-b-git-status-short.txt"), status.stdout || "");
writeFileSync(join(reports, "phase-2r-b-git-diff-stat.txt"), diffStat.stdout || "");
writeFileSync(join(reports, "phase-2r-b-base-to-working.patch"), patch.stdout || "");
...
const bodyLines = [
  "PHASE 2R-B - SOURCE MANIFEST BODY",
  "date: 2026-08-09",
  "base SHA:",
  baseSha,
...
`;
  const psFile = join(reports, "_phase2r-b-zip.ps1");
...
const reviewFiles = [
  ...
  "js/dispatcher/data-hub.js",
  "js/core/state.js",
  "translations.js",
  "docs/decisions.md",
  "AGENTS.md",
  "tests/unit/phase2r-b-language.test.mjs",
  "tests/unit/phase2r-b-lazy-plan-import.test.js",
  "tests/unit/ui-language-consistency.test.mjs",
  "tests/unit/phase2r-a311-confirm-lock-consistency.test.js",
  "tests/rules/phase2r-a311-confirm-lock-consistency.test.js",
  "scripts/phase2r-b-visual-trail.mjs",
  "scripts/phase2r-b-purge-translations.mjs",
  "scripts/phase2r-b-pack-artifacts.mjs",
  "reports/phase-2r-b-report-2026-08-09.md",
  "reports/phase-2r-b-change-ledger.md",
  MANIFEST_NAME,
  "reports/phase-2r-b-git-status-short.txt",
  "reports/phase-2r-b-git-diff-stat.txt",
  "reports/phase-2r-b-base-to-working.patch",
...
const reviewList = [...new Set([
  ...
  ...collectDir("reports/phase-2r-b-logs"),
  ...collectDir("reports/phase-2r-b-visual"),
  ...collectDir(".cursor/rules")
])].sort();
const fullList = [...new Set([...allSource, MANIFEST_NAME])].sort();

const reviewZip = join(reports, "phase-2r-b-review-source-2026-08-09.zip");
const fullZip = join(reports, "phase-2r-b-full-deliverable-2026-08-09.zip");
for (const z of [reviewZip, fullZip]) {
  if (existsSync(z)) unlinkSync(z);
}
zipWithDotNet(reviewList, reviewZip, join(reports, "_phase2r-b-review-list.txt"));
zipWithDotNet(fullList, fullZip, join(reports, "_phase2r-b-full-list.txt"));
...
`;
  const f = join(reports, "_phase2r-b-listzip.ps1");
  writeFileSync(f, ps);
