/**
 * FAZA 2R-A.3.1.1 packer — body-hash manifest + per-file SHA/size re-verify.
 */
...
const logs = join(reports, "phase-2r-a311-logs");
mkdirSync(logs, { recursive: true });
const baseSha = "a6fbcb508c67287c33479f38c3678cd44684ee60";
const MANIFEST_NAME = "reports/phase-2r-a311-source-manifest.txt";
...
function shouldSkipFile(name, relPosix) {
  ...
  if (name.startsWith("_phase2r-a311-") || name.startsWith("_tmp-")) return true;
  if (relPosix === "reports/phase-2r-a311-logs/pack.txt") return true;
  if (relPosix === "reports/phase-2r-a311-logs/manifest-verifier.txt") return true;
...
writeFileSync(join(reports, "phase-2r-a311-git-status-short.txt"), status.stdout || "");
writeFileSync(join(reports, "phase-2r-a311-git-diff-stat.txt"), diffStat.stdout || "");
writeFileSync(join(reports, "phase-2r-a311-base-to-working.patch"), patch.stdout || "");
...
const bodyLines = [
  "PHASE 2R-A.3.1.1 - SOURCE MANIFEST BODY",
  "date: 2026-08-09",
  "base SHA:",
  baseSha,
...
`;
  const psFile = join(reports, "_phase2r-a311-zip.ps1");
...
const reviewFiles = [
  ...
  "server/driver-work-policy.js",
  "js/dispatcher/plan-import.js",
  "AGENTS.md",
  "tests/unit/phase2r-a311-fail-first.test.js",
  "tests/unit/phase2r-a311-confirm-lock-consistency.test.js",
  "tests/rules/phase2r-a311-confirm-lock-consistency.test.js",
  "tests/rules/phase2r-a31-cross-writer-atomicity.test.js",
  "scripts/phase2r-a311-visual-trail.mjs",
  "scripts/phase2r-a311-pack-artifacts.mjs",
  "reports/phase-2r-a311-report-2026-08-09.md",
  "reports/phase-2r-a311-change-ledger.md",
  MANIFEST_NAME,
  "reports/phase-2r-a311-git-status-short.txt",
  "reports/phase-2r-a311-git-diff-stat.txt",
  "reports/phase-2r-a311-base-to-working.patch",
  "reports/git-status-short.txt",
  "reports/git-diff-stat.txt",
  "reports/base-to-working.patch",
  "reports/phase-2r-a311-visual/01-in-progress.png",
  "reports/phase-2r-a311-visual/02-in-progress-retry.png",
  "reports/phase-2r-a311-visual/03-recovery-required.png",
  "reports/phase-2r-a311-visual/04-xss-as-text.png",
  "reports/phase-2r-a311-visual/05-idempotent-success.png",
  "reports/phase-2r-a311-visual/TRAIL.json",
  "reports/phase-2r-a311-visual/README.md"
];

const reviewList = [...new Set([
  ...reviewFiles.filter((r) => existsSync(join(root, r))),
  ...collectDir("reports/phase-2r-a311-logs"),
  ...collectDir(".cursor/rules")
])].sort();
const fullList = [...new Set([...allSource, MANIFEST_NAME])].sort();

const reviewZip = join(reports, "phase-2r-a311-review-source-2026-08-09.zip");
const fullZip = join(reports, "phase-2r-a311-full-deliverable-2026-08-09.zip");
for (const z of [reviewZip, fullZip]) {
  if (existsSync(z)) unlinkSync(z);
}
zipWithDotNet(reviewList, reviewZip, join(reports, "_phase2r-a311-review-list.txt"));
zipWithDotNet(fullList, fullZip, join(reports, "_phase2r-a311-full-list.txt"));
...
`;
  const f = join(reports, "_phase2r-a311-listzip.ps1");
  writeFileSync(f, ps);
