/**
 * FAZA 3 D24.1 packer — body-hash manifest + per-file SHA/size re-verify.
 */
...
const logs = join(reports, "phase-3-d241-logs");
mkdirSync(logs, { recursive: true });
const baseSha = "a6fbcb508c67287c33479f38c3678cd44684ee60";
const MANIFEST_NAME = "reports/phase-3-d241-source-manifest.txt";
...
function shouldSkipFile(name, relPosix) {
  ...
  if (name.startsWith("_phase3-") || name.startsWith("_tmp-")) return true;
  if (relPosix === "reports/phase-3-d241-logs/pack.txt") return true;
  if (relPosix === "reports/phase-3-d241-logs/manifest-verifier.txt") return true;
...
writeFileSync(join(reports, "phase-3-d241-git-status-short.txt"), status.stdout || "");
writeFileSync(join(reports, "phase-3-d241-git-diff-stat.txt"), diffStat.stdout || "");
writeFileSync(join(reports, "phase-3-d241-base-to-working.patch"), patch.stdout || "");
...
const bodyLines = [
  "PHASE 3 D24.1 - SOURCE MANIFEST BODY",
  "date: 2026-08-09",
  "base SHA:",
  baseSha,
...
`;
  const psFile = join(reports, "_phase3-d241-zip.ps1");
...
const reviewFiles = [
  ...
  "docs/decisions.md",
  "AGENTS.md",
  "tests/rules/phase3-d241-assignment-resource.test.js",
  "tests/rules/phase3-d241-eid-isolation.test.js",
  "tests/unit/phase3-d241-ca-drivers-http.test.js",
  "tests/unit/phase3-assignment-integrity.test.js",
  "tests/unit/company-admin-drivers.test.mjs",
  "tests/unit/audit-log.test.js",
  "scripts/phase3-d241-pack-artifacts.mjs",
  "scripts/phase3-visual-trail.mjs",
  "reports/phase-3-d241-report-2026-08-09.md",
  "reports/phase-3-d241-change-ledger.md",
  MANIFEST_NAME,
  "reports/phase-3-d241-git-status-short.txt",
  "reports/phase-3-d241-git-diff-stat.txt",
  "reports/phase-3-d241-base-to-working.patch",
...
const reviewList = [...new Set([
  ...
  ...collectDir("reports/phase-3-d241-logs"),
  ...collectDir("reports/phase-3-visual"),
  ...collectDir(".cursor/rules")
])].sort();
const fullList = [...new Set([...allSource, MANIFEST_NAME])].sort();

const reviewZip = join(reports, "phase-3-d241-review-source-2026-08-09.zip");
const fullZip = join(reports, "phase-3-d241-full-deliverable-2026-08-09.zip");
for (const z of [reviewZip, fullZip]) {
  if (existsSync(z)) unlinkSync(z);
}
zipWithDotNet(reviewList, reviewZip, join(reports, "_phase3-d241-review-list.txt"));
zipWithDotNet(fullList, fullZip, join(reports, "_phase3-d241-full-list.txt"));
...
`;
  const f = join(reports, "_phase3-d241-listzip.ps1");
  writeFileSync(f, ps);
