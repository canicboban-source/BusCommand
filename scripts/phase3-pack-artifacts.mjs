/**
 * FAZA 3 (D24) packer — body-hash manifest + per-file SHA/size re-verify.
 */
...
const logs = join(reports, "phase-3-logs");
mkdirSync(logs, { recursive: true });
const baseSha = "a6fbcb508c67287c33479f38c3678cd44684ee60";
const MANIFEST_NAME = "reports/phase-3-source-manifest.txt";
...
function shouldSkipFile(name, relPosix) {
  ...
  if (relPosix === "reports/phase-3-logs/pack.txt") return true;
  if (relPosix === "reports/phase-3-logs/manifest-verifier.txt") return true;
...
writeFileSync(join(reports, "phase-3-git-status-short.txt"), status.stdout || "");
writeFileSync(join(reports, "phase-3-git-diff-stat.txt"), diffStat.stdout || "");
writeFileSync(join(reports, "phase-3-base-to-working.patch"), patch.stdout || "");
...
const bodyLines = [
  "PHASE 3 D24 - SOURCE MANIFEST BODY",
  "date: 2026-08-09",
  "base SHA:",
  baseSha,
...
`;
  const psFile = join(reports, "_phase3-zip.ps1");
...
const reviewFiles = [
  ...
  "js/core/bus-shift-conflicts.js",
  ...
  "tests/e2e/bus-cross-group-warn.spec.js",
  "scripts/phase3-visual-trail.mjs",
  "scripts/phase3-pack-artifacts.mjs",
  "reports/phase-3-report-2026-08-09.md",
  "reports/phase-3-change-ledger.md",
  MANIFEST_NAME,
  "reports/phase-3-git-status-short.txt",
  "reports/phase-3-git-diff-stat.txt",
  "reports/phase-3-base-to-working.patch",
...
const reviewList = [...new Set([
  ...
  ...collectDir("reports/phase-3-logs"),
  ...collectDir("reports/phase-3-visual"),
  ...collectDir(".cursor/rules")
])].sort();
const fullList = [...new Set([...allSource, MANIFEST_NAME])].sort();

const reviewZip = join(reports, "phase-3-review-source-2026-08-09.zip");
const fullZip = join(reports, "phase-3-full-deliverable-2026-08-09.zip");
for (const z of [reviewZip, fullZip]) {
  if (existsSync(z)) unlinkSync(z);
}
zipWithDotNet(reviewList, reviewZip, join(reports, "_phase3-review-list.txt"));
zipWithDotNet(fullList, fullZip, join(reports, "_phase3-full-list.txt"));
...
`;
  const f = join(reports, "_phase3-listzip.ps1");
  writeFileSync(f, ps);
