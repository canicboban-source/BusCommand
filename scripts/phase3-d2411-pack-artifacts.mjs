/**
 * FAZA 3 D24.1.1 packer — body-hash manifest + per-file SHA/size re-verify.
 */
...
const logs = join(reports, "phase-3-d2411-logs");
mkdirSync(logs, { recursive: true });
const baseSha = "a6fbcb508c67287c33479f38c3678cd44684ee60";
const MANIFEST_NAME = "reports/phase-3-d2411-source-manifest.txt";
...
const visualLatest = existsSync(join(reports, "phase-3-d2411-visual-latest.txt"))
  ? readFileSync(join(reports, "phase-3-d2411-visual-latest.txt"), "utf8").trim()
  : "";
const visualRel = visualLatest
  ? toPosix(relative(root, visualLatest))
  : "reports/phase-3-d2411-visual";

function shouldSkipFile(name, relPosix) {
  if (name.startsWith(".env")) return true;
  if (name.endsWith(".zip") && relPosix.startsWith("reports/")) return true;
  if (name.endsWith(".tmp")) return true;
  if (name.startsWith("_phase3-") || name.startsWith("_tmp-") || name.startsWith("_phase3-d2411-")) return true;
  if (relPosix === "reports/phase-3-d2411-logs/pack.txt") return true;
  if (relPosix === "reports/phase-3-d2411-logs/manifest-verifier.txt") return true;
...
writeFileSync(join(reports, "phase-3-d2411-git-status-short.txt"), status.stdout || "");
writeFileSync(join(reports, "phase-3-d2411-git-diff-stat.txt"), diffStat.stdout || "");
writeFileSync(join(reports, "phase-3-d2411-base-to-working.patch"), patch.stdout || "");
...
const bodyLines = [
  "PHASE 3 D24.1.1 - SOURCE MANIFEST BODY",
  "date: 2026-08-09",
  "base SHA:",
  baseSha,
...
`;
  const psFile = join(reports, "_phase3-d2411-zip.ps1");
...
const reviewFiles = [
  ...
  "tests/unit/company-admin-drivers.test.mjs",
  "scripts/phase3-d2411-pack-artifacts.mjs",
  "scripts/phase3-d2411-visual-trail.mjs",
  "reports/phase-3-d2411-report-2026-08-09.md",
  "reports/phase-3-d2411-change-ledger.md",
  MANIFEST_NAME,
  "reports/phase-3-d2411-git-status-short.txt",
  "reports/phase-3-d2411-git-diff-stat.txt",
  "reports/phase-3-d2411-base-to-working.patch",
...
const reviewList = [...new Set([
  ...
  ...collectDir("reports/phase-3-d2411-logs"),
...
const reviewZip = join(reports, "phase-3-d2411-review-source-2026-08-09.zip");
const fullZip = join(reports, "phase-3-d2411-full-deliverable-2026-08-09.zip");
for (const z of [reviewZip, fullZip]) {
  if (existsSync(z)) unlinkSync(z);
}
zipWithDotNet(reviewList, reviewZip, join(reports, "_phase3-d2411-review-list.txt"));
zipWithDotNet(fullList, fullZip, join(reports, "_phase3-d2411-full-list.txt"));
...
`;
  const f = join(reports, "_phase3-d2411-listzip.ps1");
  writeFileSync(f, ps);
