/**
 * FAZA 3 D24.2.1-A packer — body-hash manifest + review ⊆ full byte identity.
 */
...
const logs = join(reports, "phase-3-d2421-logs");
mkdirSync(logs, { recursive: true });
const baseSha = "a6fbcb508c67287c33479f38c3678cd44684ee60";
const MANIFEST_NAME = "reports/phase-3-d2421-source-manifest.txt";

function sha256(buf) { return createHash("sha256").update(buf).digest("hex"); }
function toPosix(rel) { return rel.split(sep).join("/"); }

const visualLatest = existsSync(join(reports, "phase-3-d2421-visual-latest.txt"))
  ? readFileSync(join(reports, "phase-3-d2421-visual-latest.txt"), "utf8").trim()
  : "";
const visualRel = visualLatest
  ? toPosix(relative(root, visualLatest))
  : "reports/phase-3-d2421-visual";
...
function shouldSkipFile(name, relPosix) {
  ...
  if (name.startsWith("_phase3-") || name.startsWith("_tmp-") || name.startsWith("_phase3-d2421-")) return true;
  if (relPosix === "reports/phase-3-d2421-logs/pack.txt") return true;
  if (relPosix === "reports/phase-3-d2421-logs/manifest-verifier.txt") return true;
...
writeFileSync(join(reports, "phase-3-d2421-git-status-short.txt"), status.stdout || "");
writeFileSync(join(reports, "phase-3-d2421-git-diff-stat.txt"), diffStat.stdout || "");
writeFileSync(join(reports, "phase-3-d2421-base-to-working.patch"), patch.stdout || "");
...
const bodyLines = [
  "PHASE 3 D24.2.1-A - SOURCE MANIFEST BODY",
  "date: 2026-08-10",
  "base SHA:",
  baseSha,
  "policy: manifest excluded from body; footer body-sha256 verifies listed content",
  "policy: pack.txt + manifest-verifier.txt excluded (written after ZIP)",
...
for (const rel of allSource) {
  ...
  hashByRel.set(rel, { hash, size: buf.length, buf });
...
`;
  const psFile = join(reports, "_phase3-d2421-zip.ps1");
...
const reviewFiles = [
  "server/driver-identity-guard.js",
  "server/company-admin-driver-ops.js",
  "server/register-company-admin-drivers.js",
  "server/driver-routes.js",
  "server/driver-csv.js",
  "js/admin/company-admin-drivers.js",
  "css/staff-desktop.css",
  "translations.js",
  "docs/decisions.md",
  "public/templates/BusCommand_Drivers_Import_v1.csv",
  "tests/rules/phase3-d242-driver-uniqueness.test.js",
  "tests/unit/phase3-d2421a-retire-company-code.test.js",
  "tests/unit/phase3-d242-driver-identity-guard.test.js",
  "tests/unit/driver-csv.test.js",
  "tests/unit/driver-credentials.test.js",
  "tests/unit/driver-company-login.test.mjs",
  "tests/e2e/ui-smoke.spec.js",
  "scripts/phase3-d2421-visual-trail.mjs",
  "scripts/phase3-d2421-pack-artifacts.mjs",
  "scripts/generate-dienstplan-blank-templates.mjs",
  "reports/phase-3-d2421-report-2026-08-10.md",
  "reports/phase-3-d2421a1-report-2026-08-10.md",
  "reports/phase-3-d2421-change-ledger.md",
  MANIFEST_NAME,
  "reports/phase-3-d2421-git-status-short.txt",
  "reports/phase-3-d2421-git-diff-stat.txt",
  "reports/phase-3-d2421-base-to-working.patch"
];

const reviewList = [...new Set([
  ...reviewFiles.filter((r) => existsSync(join(root, r))),
  ...collectDir("reports/phase-3-d2421-logs"),
  ...(existsSync(join(root, visualRel)) ? collectDir(visualRel) : []),
  ...collectDir(".cursor/rules")
])].sort();
const fullList = [...new Set([...allSource, MANIFEST_NAME])].sort();

for (const rel of reviewList) {
  if (rel === MANIFEST_NAME) continue;
  const meta = hashByRel.get(rel);
  if (!meta) throw new Error(`review path not in full body: ${rel}`);
  const buf = readFileSync(join(root, rel));
  if (sha256(buf) !== meta.hash || buf.length !== meta.size) {
    throw new Error(`review/full hash mismatch for ${rel}`);
  }
}

const reviewZip = join(reports, "phase-3-d2421-review-source-2026-08-10.zip");
const fullZip = join(reports, "phase-3-d2421-full-deliverable-2026-08-10.zip");
for (const z of [reviewZip, fullZip]) {
  if (existsSync(z)) unlinkSync(z);
}
zipWithDotNet(reviewList, reviewZip, join(reports, "_phase3-d2421-review-list.txt"));
zipWithDotNet(fullList, fullZip, join(reports, "_phase3-d2421-full-list.txt"));

const seen = new Set();
for (const { rel, hash, size } of listedMeta) {
  if (seen.has(rel)) throw new Error(`duplicate ${rel}`);
  seen.add(rel);
  const buf = readFileSync(join(root, rel));
  if (sha256(buf) !== hash || buf.length !== size) throw new Error(`hash/size mismatch ${rel}`);
  if (rel.includes("\\") || rel.includes("..") || rel.startsWith("/")) throw new Error(`unsafe ${rel}`);
}
...
`;
  const f = join(reports, "_phase3-d2421-listzip.ps1");
  writeFileSync(f, ps);
...
if (fullEntries.some((e) => e.includes("\\")) || reviewEntries.some((e) => e.includes("\\"))) {
  throw new Error("backslash in zip");
}
if (!fullEntries.includes(MANIFEST_NAME)) throw new Error("manifest missing");
if (fullEntries.some((e) => e.endsWith(".zip"))) throw new Error("nested zip");
for (const rel of reviewList) {
  if (!reviewEntries.includes(rel)) throw new Error(`review zip missing ${rel}`);
}
...
const verifyOut = [
  ...
  "review_subset_of_full_byte_identical=pass",
  "duplicates=0",
  "backslash_entries=0",
  "nested_zips=0",
  "EXIT=0"
...
writeFileSync(join(logs, "pack.txt"), [
  "PACK_OK",
  `reviewZip=${reviewZip}`,
  `fullZip=${fullZip}`,
  `review_bytes=${statSync(reviewZip).size}`,
  `full_bytes=${statSync(fullZip).size}`,
  `visual=${visualRel}`,
  verifyOut.trim()
].join("\n") + "\n");
